...
import { PieChart, BarChart2, TrendingUp, Users, CheckCircle, XCircle, Clock } from 'lucide-react';
import { useAuth } from '../../context/AuthContext';
import { useHODRequests, useDepartmentHistory } from '../../hooks/useOutpass';

export default function HODAnalytics({ requests: propRequests }) {
    const { userData } = useAuth();

    // Fetch data internally if not provided via props
    const { requests: pendingRequests, loading: pendingLoading } = useHODRequests(
        !propRequests ? userData?.department : null
    );
    const { history: historyRequests, loading: historyLoading } = useDepartmentHistory(
        !propRequests ? userData?.department : null,
        'hod'
    );

    const requests = useMemo(() => {
        if (propRequests) return propRequests;
        return [...pendingRequests, ...historyRequests];
    }, [propRequests, pendingRequests, historyRequests]);

    const isLoading = !propRequests && (pendingLoading || historyLoading);
    ...
    const analytics = useMemo(() => {
        ...
        // Status Breakdown
        const approved = requests.filter(r => r.hodStatus === 'approved').length;
        const rejected = requests.filter(r => r.hodStatus === 'rejected').length;
        const pending = requests.filter(r => r.hodStatus === 'pending').length;
        ...
        const dailyCounts = last7Days.map(date => {
            const count = requests.filter(r =>
                r.createdAt && new Date(r.createdAt).toISOString().split('T')[0] === date
            ).length;
            return { date, count };
        });
    ...
    if (isLoading) {
        return <div className="p-10 text-center text-slate-400">Loading analytics data...</div>;
    }
    ...
    return (
        <div className="p-6 space-y-8 animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="p-4 rounded-xl bg-indigo-50 border border-indigo-100 text-indigo-900">
                ...
                <div className="p-4 rounded-xl bg-emerald-50 border border-emerald-100 text-emerald-900">
                    <div className="flex items-center gap-2 mb-1 opacity-70">
                        <CheckCircle size={16} /> <span className="text-xs font-bold uppercase">Approval Rate</span>
                    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="p-6 rounded-2xl border border-slate-100 bg-white shadow-sm">
                    <h3 className="text-lg font-bold text-slate-800 mb-6 flex items-center gap-2">
                        <TrendingUp size={20} className="text-blue-500" /> Request Volume (Last 7 Days)
                    </h3>
